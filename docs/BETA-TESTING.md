...
- **Paired but nothing syncs after a reboot.** Not a Shield problem after all — see [Any device](#any-device) below.
...
- **Paired but nothing syncs, Pause does nothing, or the header says SMARTTUBE NOT LISTENING.** SmartTube's remote-control registration can silently die — first reported on a Shield after a reboot, since seen on a Google TV Streamer in the middle of a video — while YouTube keeps accepting the app's commands as if nothing were wrong. The page now says so rather than pretending. On the TV: SmartTube → Settings → **Remote control** — toggle it off and back on; if that doesn't clear it, force-stop SmartTube and reopen it, then reboot the device. Restarting the container won't help. Full steps, including what to do if SmartTube now shows a new 12-digit code, are in [TROUBLESHOOTING.md](TROUBLESHOOTING.md). If you hit this, the `docker logs` around it are worth including with your report.
