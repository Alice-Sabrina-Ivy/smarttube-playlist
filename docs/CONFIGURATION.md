...
| `RESET_LOUNGE` | (unset) | Set to `1` to clear **only the SmartTube pairing** on the next start; the TV-remote pairing is kept, so you land on the *Pair with SmartTube* card rather than the setup screen. For when SmartTube shows a new 12-digit code and the old pairing can never work again. Same one-shot rule as `RESET_PAIRING`. See [SECURITY.md](../SECURITY.md#resetting-only-the-smarttube-pairing) |
