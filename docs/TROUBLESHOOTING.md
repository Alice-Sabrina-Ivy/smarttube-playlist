...
**Pause does nothing, a video disappears from the page while it keeps playing on the TV, or the header says SMARTTUBE NOT LISTENING.** SmartTube's own link to YouTube — its *Remote control* registration — has died on the TV, while YouTube carries on accepting this app's commands as if nothing were wrong. The TV is fine and the video is fine; the app just can't see or pause it. Fix it on the TV, in this order, checking the page after each step: **SmartTube → Settings → Remote control** — switch it off, then on again. If that doesn't do it, force-stop SmartTube (Settings → Apps → See all apps → SmartTube → **Force stop**) and open it again. Still not listening? Reboot the streaming device. The page catches up on its own within a few minutes, or the moment anyone adds a video while SmartTube is still up on the TV. **Restarting the container won't help** — it reconnects to the same dead link — and it empties the queue. If Remote control now shows a *new* 12-digit code, SmartTube has a new identity and the old pairing can never work again: set `RESET_LOUNGE` to `1` (see the note at the top for how, on your install), restart, and pair again from the *Pair with SmartTube* card. Your TV pairing is untouched, and like `RESET_PAIRING` the flag fires once. In the logs this state reads `Lounge connected: None` on every reconnect — a healthy one names the device, like `Lounge connected: Google TV Streamer (SmartTube)`. The first time, a `WARNING` line follows that spells out what it means and repeats the fix; later reconnects just add `SmartTube is still not in the lounge`.
